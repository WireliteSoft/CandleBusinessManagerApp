...
In the Cloudflare dashboard, create the D1 database and apply the SQL files in `migrations` in numeric order. Then run every file in `exports/d1-import-chunks/` through the D1 console in filename order. The chunks contain only plain `INSERT` statements because the dashboard console rejects SQL transaction wrappers. Keep the export files private: they contain customer and business data and are intentionally excluded from Git.
